...
HOUGH_THRESHOLD  = 15          # Lowered: sparse edges still accumulate votes
HOUGH_RHO        = 1
HOUGH_THETA      = np.pi / 180

# Slope filter — highway lanes are shallower than city streets
MIN_SLOPE_ABS    = 0.2         # was 0.3 — catches shallower highway lanes
...
ROI_TOP_FRAC        = 0.6     # top of trapezoid as fraction of frame height
ROI_TOP_WIDTH_FRAC  = 0.08     # half-width of narrow top
ROI_BOTTOM_FRAC     = 0.82     # bottom — above the car hood
ROI_BOTTOM_MARGIN   = 0.12     # inset from left/right edges at the bottom
...
def build_roi_mask(frame_shape):
    """
    Trapezoidal ROI. All values are frame-relative so it works at any resolution.

    The bottom corners are inset from the frame edges to exclude the
    car body / side mirrors that appear in wide-angle dashcam footage.
    """
    h, w = frame_shape[:2]
    cx = w // 2
    ...
    top_hw   = int(w * ROI_TOP_WIDTH_FRAC)
    bot_inset = int(w * ROI_BOTTOM_MARGIN)

    pts = np.array([
        [bot_inset,       bottom_y],   # bottom-left  (inset from edge)
        [cx - top_hw,     top_y   ],   # top-left
        [cx + top_hw,     top_y   ],   # top-right
        [w - bot_inset,   bottom_y],   # bottom-right (inset from edge)
    ], dtype=np.int32)
...
def classify_lines(lines, frame_shape):
    """
    Classify Hough lines into left / right candidates.

    KEY FIX from v1: we now evaluate each line's x-position at the
    BOTTOM of the ROI (y = ROI_BOTTOM_FRAC * height), not at y=0.
    y=0 is above the frame for most highway lane lines, making the
    x-intercept unreliable.  The bottom of the ROI is always within
    the visible image, giving a stable position reference.

    Classification:
      LEFT  lane: negative slope  AND  x_at_bottom < frame_center
      RIGHT lane: positive slope  AND  x_at_bottom >= frame_center
    """
    h, w = frame_shape[:2]
    cx   = w / 2.0
    y_ref = int(h * ROI_BOTTOM_FRAC)   # reference y for x-position check
    ...
    for line in lines:
        ...
        # x-position of the line at the bottom of the ROI
        x_at_bottom = (y_ref - b) / m

        if m < 0 and x_at_bottom < cx:
            left_params.append((m, b))
        elif m > 0 and x_at_bottom >= cx:
...
def draw_hough_lines_debug(frame, lines, frame_shape):
    """Draw ALL raw Hough lines in dim cyan for debugging."""
    if lines is None:
        return
    h, w = frame_shape[:2]
    for line in lines:
        ...
        if m is None:
            continue
        y1, y2 = int(h * ROI_TOP_FRAC), int(h * ROI_BOTTOM_FRAC)
...
def process_video(video_path, show_debug=True):
    ...
    y_bottom = int(h * ROI_BOTTOM_FRAC)
